...
def main():
    ...
    for name, X in representations.items():
        ...
        print(f"{name:<20} {res['n_features']:>6} feats  "
              f"Acc={res['balanced_acc']:.3f}  AUC={res['roc_auc']:.3f}")
    ...
    results.append(res)
    print(f"  → {X_hybrid_genomic.shape}  Acc={res['balanced_acc']:.3f}  AUC={res['roc_auc']:.3f}")
    ...
    results.append(res)
    print(f"  → {X_hybrid_classical_multi.shape}  Acc={res['balanced_acc']:.3f}  AUC={res['roc_auc']:.3f}")
    ...
    results.append(res)
    print(f"  → {X_hybrid_full_classical.shape}  Acc={res['balanced_acc']:.3f}  AUC={res['roc_auc']:.3f}")
    ...
    for model_name, model_data in sota_models.items():
        ...
        print(f"  Bal_Acc={res['balanced_acc']:.3f}  AUC={res['roc_auc']:.3f}")
